...
- `lazy_api_check` (Boolean) If set to true, the provider will not connect to the controller during planning. This allows for planning the creation of resources without a live controller. However, the provider will still connect to the controller when planning for updates.Note that the validators/checks (e.g. check some resources are only valid with JAAS) will not be run until apply time.
