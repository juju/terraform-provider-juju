...
The provider exposes a `default_timeouts` block that controls how long the
...
```terraform
provider "juju" {
  controller_addresses = "10.225.205.241:17070"
  username             = "jujuuser"
  password             = "password1"
...
Precedence: a resource-level `timeouts` block wins over `default_timeouts`,
which wins over the built-in defaults (30 minutes for create/update, 15
minutes for delete).
...
- `default_timeouts` (Attributes) Default timeouts for resource operations. These apply to all resources unless overridden by a resource-level `timeouts` block. Values are duration strings, e.g. "60m". (see [below for nested schema](#nestedblock--default_timeouts))
- `lazy_api_check` (Boolean) If set to true, the provider will not connect to the controller during planning for creation. This allows for planning the creation of resources without a live controller. However, the provider will still connect to the controller when planning for updates. Note that the validators/checks will not be run until apply time - this is relevant for some resources that are only valid with JAAS.
...
### Nested Schema for `default_timeouts`
...
### Nested Schema for `offering_controllers`
