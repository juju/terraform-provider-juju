		...
		Steps: []resource.TestStep{
			...
			// Update in-place every other field (they're all update in place).
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckTypeSetElemNestedAttrs(resourceName, "regions.*", map[string]string{"name": "us-east-1"}),
					resource.TestCheckTypeSetElemNestedAttrs(resourceName, "regions.*", map[string]string{"name": "us-east-2"}),
					resource.TestCheckTypeSetElemNestedAttrs(resourceName, "regions.*", map[string]string{"name": "default", "endpoint": "https://region-default.example.com", "identity_endpoint": "https://identity-default.example.com", "storage_endpoint": "https://storage-default.example.com"}),
				),
			},
