	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			// Change order of auth-types.
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(resourceName, "auth_types.#", "2"),
					resource.TestCheckResourceAttr(resourceName, "auth_types.1", "userpass"),
					resource.TestCheckResourceAttr(resourceName, "auth_types.0", "access-key"),
				),
			},
			// Change order of ca certs.
			{
				Config: testAccCloudFromTemplate(internaltesting.TemplateData{
					"Name":                    cloudName,
					"Type":                    "openstack",
					...
					"StorageEndpoint":         "https://storage.example.com",
					"IncludeCACerts":          true,
					"CACertsHCL":              hclCACerts([]string{ca2, ca1}),
					"IncludeRegions":          true,
					"RegionsHCL": hclRegions([]map[string]string{
						{"name": "default", "endpoint": "https://region-default.example.com", "identity_endpoint": "https://identity-default.example.com", "storage_endpoint": "https://storage-default.example.com"},
						{"name": "us-east-1"},
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(resourceName, "ca_certificates.#", "2"),
					resource.TestCheckResourceAttr(resourceName, "ca_certificates.0", ca2+"\n"),
					resource.TestCheckResourceAttr(resourceName, "ca_certificates.1", ca1+"\n"),
				),
			},
			// Change order of regions and append a new one.
			{
				Config: testAccCloudFromTemplate(internaltesting.TemplateData{
					"Name":                    cloudName,
					"Type":                    "openstack",
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "regions.0.identity_endpoint", "https://identity-default.example.com"),
					resource.TestCheckResourceAttr(resourceName, "regions.0.storage_endpoint", "https://storage-default.example.com"),
					resource.TestCheckResourceAttr(resourceName, "regions.1.name", "us-east-2"),
					resource.TestCheckResourceAttr(resourceName, "regions.2.name", "us-east-1"),
