...
type CommandRunner interface {
	...
	// Version returns the Juju CLI's self-reported version.
...
func (d *DefaultJujuCommand) Bootstrap(ctx context.Context, args BootstrapArguments) (*ControllerConnectionInformation, error) {
	...
	tflog.SubsystemDebug(ctx, LogJujuCommand, fmt.Sprintf("Log file from bootstrap: %s\n", runner.LogFilePath()))
...
func (d *DefaultJujuCommand) Destroy(ctx context.Context, args DestroyArguments) error {
	...
	tflog.SubsystemDebug(ctx, LogJujuCommand, fmt.Sprintf("Log file for destroy: %s\n", runner.LogFilePath()))
...
func performDestroy(ctx context.Context, args DestroyArguments, runner CommandRunner) error {
	...
	if err := checkVersionsMatch(cliVersion, args.ConnectionInfo.AgentVersion); err != nil {
...
func GetCloudInformation(ctx context.Context, connInfo *ControllerConnectionInformation) (*CloudInformation, error) {
	...
	mi, err := modelmanagerAPI.ModelInfo(ctx, []names.ModelTag{names.NewModelTag(cfg.UUID())})
	if err != nil {
	...
	credentials, err := cloudAPI.CredentialContents(ctx, cloudTag.Id(), cloudCredentialTag.Name(), true)
	...
	cloud, err := cloudAPI.Cloud(ctx, cloudTag)
	if err != nil {
