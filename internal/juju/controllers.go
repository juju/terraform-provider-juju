...
func GetCloudInformation(ctx context.Context, connInfo *ControllerConnectionInformation) (*CloudInformation, error) {
	...
	credentials, err := cloudAPI.Credentials(cloudCredentialTag)
	if err == nil && len(credentials) == 1 {
		authType = string(credentials[0].Result.AuthType)
		credentialAttrs = credentials[0].Result.Attributes
	}
	...
	for _, at := range cloud.AuthTypes {
