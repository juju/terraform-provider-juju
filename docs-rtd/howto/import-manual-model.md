...
myst:
  html_meta:
    description: "Learn how to import a manually deployed Juju model into a Terraform plan using terraform query, juju-tf-refwriter, and an AI agent with the model export fixup skill."
---

(import-a-manually-created-model)=
# Import a manually deployed model into a Terraform plan

If you have a Juju model that was created outside Terraform (e.g. with the Juju CLI) and you want to bring it under Terraform management, `terraform query` can generate the configuration, and a combination of `tf-refwriter` and an AI agent using the [model export fixup](https://github.com/juju/terraform-provider-juju/blob/main/.github/instructions/tf-model-export-fixup.instructions.md) skill can refine it into a clean, maintainable plan.
...
## 4. Resolve cross-references with juju-tf-refwriter

The generated config uses literal UUIDs and names instead of references between resources. The `juju-tf-refwriter` tool rewrites them mechanically. Run it directly with Go tooling:

```bash
go run github.com/juju/terraform-provider-juju/juju-tf-refwriter exported.tf
```

## 5. Set up the exported config for `terraform plan`
...
## 6. Load the model export fixup skill

The [model export fixup skill](https://github.com/juju/terraform-provider-juju/blob/main/.github/instructions/tf-model-export-fixup.instructions.md) guides an AI agent through the remaining fixes: removing unmanageable resources, resolving drift, handling controller-set defaults, and splitting the result into maintainable files.

Download the skill file (or point your agent at the URL) and invoke it on `exported.tf`.

## 7. Iterate with the agent
...
## 8. Decide when it's good enough
