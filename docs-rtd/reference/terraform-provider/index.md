...
- `default_timeouts` (Block, Optional) Default timeouts for resource operations. These apply to all resources unless overridden by a resource-level `timeouts` block. Values are duration strings, e.g. "60m". (see [below for nested schema](#nestedblock--default_timeouts))
